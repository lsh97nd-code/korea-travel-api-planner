...
| `x` | `x` | 장소의 경도(Longitude) |
| `y` | `y` | 장소의 위도(Latitude) |

Kakao Local API에서 제공하는 `x`, `y` 값은 문자열 형태로 반환될 수 있으므로 프로그램에서 `float`형으로 변환하여 저장합니다.

따라서 각 맛집 정보는 다음과 같은 구조로 저장합니다.
...
Kakao Local API에서 제공하는 `x`, `y` 값은 문자열 형태로 반환될 수 있으므로 프로그램에서 숫자형으로 변환하여 저장합니다.
...
입력한 날짜는 `YYYY-MM-DD` 형식인지 확인하고, 실제 달력에 존재하는 날짜인지 검증한 후 프로그램에서 사용합니다.
...
![프로그램 정상 실행 결과](evidence/09-0_정상실행결과.jpg)

**그림 9-2. 여행 날짜 입력 후 프로그램 정상 실행 결과**

`2026-09-15`를 입력하여 프로그램을 실행한 결과 여행지 추천이 수행되고, 추천 지역에 대한 Kakao Local API 맛집 검색 결과가 정상적으로 출력되는 것을 확인하였다.
...
`-date` 옵션으로 입력받은 날짜는 다음 두 단계로 검증합니다.

1. `YYYY-MM-DD` 형식인지 확인
2. 실제 달력에 존재하는 날짜인지 확인

따라서 날짜의 문자 형식이 잘못된 경우뿐만 아니라 형식은 올바르지만 실제로 존재하지 않는 날짜도 정상적인 여행 날짜로 처리하지 않습니다.


#### 9-3-1. 잘못된 날짜 형식 검증

예를 들어 다음과 같이 `YYYY-MM-DD` 형식과 다른 날짜를 입력할 수 있습니다.

```powershell
python travel_planner.py -date "2026/09/15"
```

실제 테스트에서는 `2026/09/15`를 입력하여 잘못된 날짜 형식에 대한 오류 처리가 정상적으로 동작하는지 확인하였습니다.

실행 결과 다음과 같은 오류 메시지가 출력되었습니다.

```text
"2026/09/15"는 올바른 날짜 형식이 아닙니다. "YYYY-MM-DD" 형식으로 입력하세요.
```

이를 통해 `/`를 사용한 잘못된 날짜 형식을 프로그램이 감지하고, 여행지 추천 API를 호출하지 않은 상태에서 프로그램을 종료하는 것을 확인하였습니다.

![잘못된 날짜 형식 오류 처리](evidence/09-1_날짜형식오류.png)

**그림 9-3-1. 잘못된 날짜 형식 입력 오류 처리 결과**

`YYYY-MM-DD` 형식이 아닌 `2026/09/15`를 입력한 결과, 올바른 날짜 형식을 안내하는 오류 메시지가 출력되고 프로그램이 종료되는 것을 확인하였다.


#### 9-3-2. 존재하지 않는 날짜 검증

날짜가 `YYYY-MM-DD` 형식에 맞더라도 실제 달력에 존재하지 않는 날짜는 허용하지 않습니다.

실제 테스트에서는 다음과 같이 `2026-02-30`을 입력하였습니다.

```powershell
python travel_planner.py -date "2026-02-30"
```

`2026-02-30`은 `YYYY-MM-DD` 형식 자체는 올바르지만, 2026년 2월에는 30일이 존재하지 않으므로 실제로 사용할 수 없는 날짜입니다.

실행 결과 다음과 같은 오류 메시지가 출력되었습니다.

```text
usage: travel_planner.py [-h] -date YYYY-MM-DD
travel_planner.py: error: argument -date/-d/--date: "2026-02-30"는 존재하지 않는 날짜입니다. "YYYY-MM-DD" 형식의 올바른 날짜를 입력하세요.
```

이를 통해 프로그램이 날짜의 문자 형식만 확인하는 것이 아니라 실제 달력에 존재하는 날짜인지까지 검증하는 것을 확인하였습니다.

또한 유효하지 않은 날짜가 입력된 경우 여행지 추천 API를 호출하지 않고 프로그램이 종료됩니다.

![존재하지 않는 날짜 오류 확인 결과](evidence/09-3_존재하지않는날짜오류확인결과.png)

**그림 9-3-2. 존재하지 않는 날짜 입력 오류 확인 결과**

`YYYY-MM-DD` 형식은 올바르지만 실제로 존재하지 않는 `2026-02-30`을 입력한 결과, 프로그램이 해당 날짜를 유효하지 않은 날짜로 판단하여 오류 메시지를 출력하고 종료하는 것을 확인하였다.

따라서 날짜 입력 검증 테스트를 통해 다음 두 가지 오류 상황이 서로 구분되어 처리되는 것을 확인하였습니다.

| 테스트 항목 | 입력값 | 검증 결과 |
|---|---|---|
| 잘못된 날짜 형식 | `2026/09/15` | `YYYY-MM-DD` 형식 오류를 안내하고 종료 |
| 존재하지 않는 날짜 | `2026-02-30` | 실제 존재하지 않는 날짜임을 안내하고 종료 |
...
필수 옵션은 프로그램을 실행하기 위해 반드시 입력해야 하는 값을 의미합니다. 이 프로그램에서는 여행 날짜가 있어야 여행지 추천을 수행할 수 있으므로 `-date` 옵션을 필수로 지정하였습니다.

따라서 다음과 같이 `-date`를 입력하지 않고 프로그램을 실행하여 필수 옵션 누락 상황을 테스트하였습니다.

```powershell
python travel_planner.py
```

실행 결과 다음과 같은 메시지가 출력되었습니다.

```text
usage: travel_planner.py [-h] -date YYYY-MM-DD
travel_planner.py: error: the following arguments are required: -date/-d/--date
```

여기서 `the following arguments are required`는 **"다음 인수(옵션)는 필수입니다"**라는 의미입니다.

`arguments`는 프로그램을 실행할 때 명령행에서 전달하는 입력값이나 옵션을 의미하고, `required`는 반드시 입력해야 한다는 의미입니다.

따라서 `the following arguments are required: -date/-d/--date`는 **여행 날짜를 전달하는 `-date` 옵션이 입력되지 않았으므로 해당 옵션을 반드시 입력해야 한다는 오류 안내**입니다.

실제 테스트에서도 `python travel_planner.py`와 같이 `-date` 옵션을 완전히 제외하고 실행한 결과, 프로그램이 필수 옵션 누락을 감지하여 사용법과 오류 메시지를 출력한 후 종료하는 것을 확인하였습니다.

![필수 date 옵션 누락 오류 처리](evidence/09-2_필수옵션누락.png)

**그림 9-4. 필수 `-date` 옵션 누락 오류 처리 결과**

`-date` 옵션을 입력하지 않고 프로그램을 실행한 결과 `the following arguments are required: -date/-d/--date` 오류 메시지가 출력되었으며, 이를 통해 필수 여행 날짜가 없는 상태에서 다음 처리 단계로 진행되는 것을 방지하는 것을 확인하였다.
...
Kakao Local API에서 제공하는 `x`, `y` 값은 문자열 형태로 반환될 수 있으므로 프로그램에서 `float`형으로 변환하여 숫자형으로 저장합니다.
...
이를 통해 각 추천 지역의 맛집 정보뿐만 아니라 장소의 위치 좌표까지 함께 원본 JSON에 저장할 수 있도록 구성하였습니다.
...
최종 여행 안내서 생성이 완료되면 `save_results()` 함수를 이용하여 원본 JSON과 Markdown 여행 안내서를 `results/` 폴더에 저장합니다.
...
원본 JSON에는 여행 날짜, 여행지 추천 결과, 지역별 맛집 검색 결과 및 오류 정보가 저장됩니다.

Markdown 파일에는 추천 지역별 여행 정보와 오전·오후·저녁 일정이 포함된 최종 여행 안내서가 저장됩니다.

파일 저장이 완료되면 터미널에 `[저장 완료]` 메시지와 두 파일의 저장 경로를 출력합니다.

![결과 파일 저장 완료](evidence/14-6_결과파일저장완료.png)

**그림 14-6. 원본 JSON 및 Markdown 결과 파일 저장 완료**

프로그램 실행이 완료된 후 `results/` 폴더에 원본 JSON과 Markdown 여행 안내서가 저장되고 터미널에서 각 결과 파일의 저장 경로가 출력된 것을 확인하였다.
...
- 각 지역에서 최대 5개의 맛집 정보를 검색하고 `place_name`, `address`, `category`, `url`, `x`, `y` 정보를 저장한다.
- `x`는 경도(Longitude), `y`는 위도(Latitude)를 의미하며, Kakao Local API에서 받은 값을 프로그램에서 숫자형으로 변환하여 저장한다.
...
각 지역에 대해 최대 5개의 맛집을 검색하도록 구성하였으며, 검색된 경우 맛집명, 주소, 음식점 분류, Kakao 지도 URL 및 위치 좌표인 `x`(경도), `y`(위도)를 저장한다.
...
단, 기존 `15-3_지역별맛집JSON저장.png`가 `x`, `y` 추가 이전에 촬영한 증빙이라면 좌표 저장 여부는 새로운 실행 결과를 확인한 후 증빙 화면을 갱신한다.
...
`x`는 경도(Longitude), `y`는 위도(Latitude)를 의미하며, Kakao Local API에서 받은 좌표 값을 프로그램에서 숫자형으로 변환하여 저장한다.
...
이와 같이 여행지 추천 정보와 지역별 맛집 정보를 분리하여 저장함으로써 복수 지역의 추천 결과와 각 지역의 맛집 검색 결과를 명확하게 구분하여 확인할 수 있도록 하였다.

또한 Kakao Local API에서 제공하는 위치 좌표 `x`, `y`를 함께 저장하여 각 맛집의 실제 위치 정보를 원본 JSON에서 확인할 수 있도록 구성하였다.
...
각 지역에서는 최대 5개의 맛집 정보를 검색하며, 검색 결과에서 `place_name`, `address`, `category`, `url`, `x`, `y`를 추출한다.

Kakao Local API에서 제공하는 `x`는 경도(Longitude), `y`는 위도(Latitude)를 의미하며, 프로그램에서는 해당 값을 숫자형으로 변환하여 저장한다.

`search_restaurants()` 함수에서 맛집 정보를 저장하는 핵심 코드는 다음과 같다.
...
검색된 맛집 목록은 추천 지역명을 키로 사용하는 `restaurants_by_city`에 저장한다.
...
특정 지역에서 맛집 검색 결과가 없으면 `데이터 없음`으로 처리한다.

또한 Kakao Local API 요청 중 네트워크·HTTP 오류 또는 응답 구조 오류가 발생하면 해당 오류 내용을 `errors`에 기록하고 해당 지역의 맛집 목록을 빈 목록으로 처리한 뒤 다른 지역의 처리를 계속하도록 구현하였다.
...
프로그램의 주요 기능과 오류 처리 기능이 의도한 대로 동작하는지 확인하기 위해 정상 실행, 입력값 검증, API Key 미설정, LLM 구조화 결과 처리 실패, 장소 API 오류, 캐싱 및 복수 지역 추천 기능을 테스트하였습니다.
...
| 날짜 형식 오류 | 올바른 날짜 입력 방법을 안내하고 프로그램 종료 | `2026/09/19`를 입력하여 실행한 결과 올바른 날짜 형식이 아니라는 오류와 `"YYYY-MM-DD" 형식으로 입력하세요.`라는 안내가 출력되었으며 프로그램이 종료됨 |
| 필수 `-date` 옵션 누락 | 사용법 또는 오류 메시지를 출력하고 프로그램 종료 | `-date` 옵션 없이 `python travel_planner.py`를 실행한 결과 필수 인자가 누락되었다는 오류 메시지와 CLI 사용 방법이 출력되었으며, API 호출 전에 프로그램이 종료됨 |
...
| LLM 구조화 결과 처리 실패 | 구조화 결과 처리에 실패하면 보정 Prompt를 적용하여 최대 1회 재시도하고, 재시도 성공 시 다음 단계 진행 | 첫 번째 요청에서 구조화 결과 처리 실패 상황을 발생시킨 결과 재시도 안내 메시지가 출력되었으며, 보정 Prompt를 적용한 1회 재시도 후 정상적인 여행지 추천 결과가 생성되어 다음 단계로 진행됨 |
...
프로그램 실행 결과 입력한 여행 날짜와 API Key 설정 상태가 확인되었으며, OpenAI API를 이용한 여행지 추천과 Kakao Local API를 이용한 지역별 맛집 검색이 정상적으로 진행되었습니다.

이후 추천 정보와 맛집 정보를 결합한 최종 Markdown 여행 안내서가 생성되었으며 다음 결과 파일이 `results/` 폴더에 저장되는 것을 확인하였습니다.
...
이를 통해 정상적인 날짜와 API Key가 입력된 경우 프로그램의 전체 처리 과정이 정상적으로 완료되는 것을 확인하였습니다.
...
### 17-3. LLM 구조화 결과 처리 실패 및 재시도 테스트

LLM의 여행지 추천 결과를 정상적인 구조화 형태로 처리하지 못하는 상황에 대한 오류 처리를 확인하기 위해 첫 번째 요청에서 구조화 결과 처리 실패 상황을 발생시켰습니다.

첫 번째 처리 실패 후 프로그램이 무한 재시도를 수행하지 않고 최대 1회만 다시 요청하도록 구성하였습니다.

최신 구현에서는 동일한 요청을 그대로 반복하는 것이 아니라 재시도 시 보정 Prompt를 추가하여 다음 조건을 다시 명확하게 지정합니다.

- 설명 문장이나 Markdown 코드블록을 추가하지 않음
- `recommendations`에 정확히 3개의 항목을 포함
- 각 항목에 `recommended_city`, `weather`, `events`, `reason`을 포함
- `recommended_city`, `weather`, `reason`은 문자열
- `events`는 문자열 배열
...
보정 Prompt 적용
    ↓
필수 Key·자료형·추천 항목 수 재지정
    ↓
2차 LLM 요청
    ↓
TravelRecommendations 구조 검증
```

이후 1회 재시도에서 정상적인 여행지 추천 결과가 생성되면 다음 처리 단계로 진행합니다.

재시도 후에도 구조화된 결과를 처리하지 못하는 경우에는 오류를 발생시키고 프로그램을 종료하도록 구성하였습니다.

이를 통해 LLM 결과 처리 오류가 발생했을 때 무한 반복하지 않고 제한된 횟수만 재시도하며, 재시도 시에는 구조화 결과를 얻기 위한 조건을 보다 명확하게 지정하도록 구현하였습니다.

해당 테스트 결과는 13-3의 LLM 구조화 결과 처리 실패 및 재시도 증빙을 통해 확인할 수 있습니다.
...
**그림 17-6-1. Kakao Local API 맛집 검색 성공 실행 결과**

위 실행 결과에서 여행 날짜 입력과 API Key 설정 확인 후 여행지가 추천되었으며, 추천 지역을 기준으로 맛집 검색 결과가 정상적으로 출력되는 것을 확인하였습니다.

![맛집 검색 5개 출력 결과](evidence/10-2_맛집검색성공_5개출력결과.png)

**그림 17-6-2. Kakao Local API 맛집 5개 출력 결과**

검색 결과 총 5곳의 맛집이 출력되었으며, 각 맛집에 대해 맛집명, 주소, 음식점 분류 및 Kakao 지도 URL이 정상적으로 표시되는 것을 확인하였습니다.

이를 통해 OpenAI API에서 추천된 지역명을 장소 검색 단계의 입력값으로 사용하고, Kakao Local API에서 검색된 맛집 정보를 프로그램에서 정상적으로 처리하는 기능이 동작함을 확인하였습니다.


### 17-7. 복수 지역 추천 테스트

`2026-09-15`를 기준으로 여행지 추천 기능을 실행한 결과 다음 세 지역이 순차적으로 추천되었습니다.

- 제주시
- 경주시
- 강릉시

첫 번째 추천 지역으로 제주시가 출력되는 것을 확인하였습니다.

![복수 지역 추천 - 제주시](evidence/15-1_복수지역추천_제주시.png)

**그림 17-7-1. 복수 지역 추천 결과 - 제주시**

`[추천 지역 1]`에서 첫 번째 추천 지역으로 제주시가 출력되는 것을 확인하였습니다.

두 번째 추천 지역으로 경주시가 출력되는 것을 확인하였습니다.

![복수 지역 추천 - 경주시](evidence/15-2_복수지역추천_경주시.png)

**그림 17-7-2. 복수 지역 추천 결과 - 경주시**

`[추천 지역 2]`에서 두 번째 추천 지역으로 경주시가 출력되는 것을 확인하였습니다.

세 번째 추천 지역으로 강릉시가 출력되는 것을 확인하였습니다.

![복수 지역 추천 - 강릉시](evidence/15-3_복수지역추천_강릉시.png)

**그림 17-7-3. 복수 지역 추천 결과 - 강릉시**

`[추천 지역 3]`에서 세 번째 추천 지역으로 강릉시가 출력되는 것을 확인하였습니다.

각 추천 지역의 정보는 `recommendations` 배열에 저장되며, 각 지역의 `recommended_city` 값을 맛집 검색 단계의 입력값으로 사용합니다.

검색 단계에서는 `normalize_city_name()`을 통해 추천 지역명을 시·군 단위로 정규화한 뒤 `PlaceProvider` 인터페이스를 구현한 `KakaoPlaceProvider`를 통해 Kakao Local API 맛집 검색을 수행하도록 구성하였습니다.
...
이를 통해 하나의 여행 날짜 `2026-09-15`에 대해 제주시, 경주시, 강릉시의 서로 다른 국내 여행지 3곳을 추천하고 각 지역의 데이터를 순차적으로 처리하는 복수 지역 추천 기능이 정상적으로 동작하는 것을 확인하였습니다.


### 17-8. 날짜 형식 오류 테스트

여행 날짜 입력값이 `YYYY-MM-DD` 형식을 따르지 않는 경우 프로그램이 잘못된 입력을 감지하고 API 호출 전에 종료되는지 확인하였습니다.

테스트 명령어:

```powershell
python travel_planner.py -date "2026/09/19"
```

실행 결과 `2026/09/19`는 올바른 날짜 형식이 아니라는 오류 메시지가 출력되었으며, `YYYY-MM-DD` 형식으로 입력하도록 안내하는 것을 확인하였습니다.

![날짜 형식 오류 테스트 결과](evidence/28-1_날짜형식오류.png)

**그림 17-8-1. 날짜 형식 오류 입력 검증 결과**

잘못된 날짜 형식인 `2026/09/19`를 입력한 결과 다음과 같은 오류 메시지가 출력되었습니다.

```text
travel_planner.py: error: argument -date/-d/--date: "2026/09/19"는 올바른 날짜 형식이 아닙니다. "YYYY-MM-DD" 형식으로 입력하세요.
```

이를 통해 잘못된 날짜 형식이 입력된 경우 프로그램이 이를 감지하고 API 호출 전에 실행을 종료하는 것을 확인하였습니다.


### 17-9. 필수 `-date` 옵션 누락 테스트

여행 날짜를 지정하는 필수 `-date` 옵션을 입력하지 않은 경우 프로그램이 누락된 필수 인자를 감지하고 API 호출 전에 종료되는지 확인하였습니다.
...
실행 결과 필수 `-date` 옵션이 누락되었다는 오류 메시지와 함께 CLI 사용 방법이 출력되는 것을 확인하였습니다.

![필수 -date 옵션 누락 테스트 결과](evidence/28-2_-date옵션누락오류.png)

**그림 17-9-1. 필수 `-date` 옵션 누락 입력 검증 결과**

`-date` 옵션을 입력하지 않고 프로그램을 실행한 결과 다음과 같은 오류 메시지가 출력되었습니다.
...
이를 통해 필수 여행 날짜 옵션이 입력되지 않은 경우 프로그램이 누락된 인자를 감지하고 API 호출 전에 실행을 종료하는 것을 확인하였습니다.


### 17-10. 맛집 검색 결과 0건 처리 테스트

Kakao Local API의 맛집 검색 결과가 0건인 경우 프로그램이 중단되지 않고 해당 지역의 맛집 정보를 `데이터 없음`으로 처리한 뒤 다음 추천 지역의 처리를 계속하는지 확인하였습니다.

여행 날짜 `2026-09-18`을 입력하여 프로그램을 실행하였습니다.

테스트 명령어:

```powershell
python travel_planner.py -date "2026-09-18"
```

실행 결과 여행지 추천은 정상적으로 수행되었으며, 추천 지역의 맛집 검색 결과가 없는 경우 다음과 같이 `데이터 없음`으로 출력되는 것을 확인하였습니다.
...
맛집 검색 결과가 0건이더라도 프로그램이 종료되지 않고 다음 추천 지역의 여행 정보 처리로 계속 진행되는 것을 확인하였습니다.

![맛집 검색 결과 0건 처리](evidence/27.%20맛집검색0건처리Test결과%20(01).png)

**그림 17-10-1. 맛집 검색 결과 0건 처리 및 다음 추천 지역 처리**

위 실행 결과에서 제주시의 맛집 검색 결과가 `데이터 없음`으로 표시되었습니다.

이후 프로그램이 중단되지 않고 다음 추천 지역인 강릉시의 추천 정보와 행사·활동 등의 처리를 계속하는 것을 확인하였습니다.

이를 통해 특정 추천 지역에서 맛집 검색 결과가 없더라도 해당 지역을 제외하거나 프로그램 전체를 종료하지 않고 다음 처리 단계로 진행하도록 구현된 것을 확인하였습니다.

또한 맛집 정보가 없는 상태에서도 최종 여행 안내서 생성 단계까지 정상적으로 진행되는지 확인하였습니다.

실행 결과 최종 여행 안내서가 생성되었으며 다음 파일이 `results/` 폴더에 정상적으로 저장되었습니다.

```text
results/2026-09-18_raw.json
results/2026-09-18_travel_guide.md
```

![맛집 검색 결과 0건 최종 완료](evidence/27.%20맛집검색0건처리Test결과%20(02).png)

**그림 17-10-2. 맛집 검색 결과 0건 발생 후 최종 결과 저장 완료**

위 실행 결과에서 맛집 정보가 제공되지 않은 경우에도 오전·오후·저녁 일정과 여행 준비 공통 체크리스트를 포함한 최종 여행 안내서가 생성되는 것을 확인하였습니다.

또한 프로그램 마지막에 `[저장 완료]`가 출력되었으며 원본 JSON과 Markdown 여행 안내서가 다음과 같이 정상 저장되었습니다.

```text
[저장 완료]
원본 데이터: results\2026-09-18_raw.json
여행 안내서: results\2026-09-18_travel_guide.md
```

따라서 맛집 검색 결과가 0건인 경우의 전체 처리 흐름은 다음과 같습니다.
...
최종 여행 안내서 생성
    ↓
원본 JSON 및 Markdown 파일 저장
```

이를 통해 맛집 검색 결과가 없는 상황을 비치명적 상황으로 처리하여 프로그램 전체 실행을 계속하고, 최종 결과 파일까지 정상적으로 생성하는 기능이 동작하는 것을 확인하였습니다.


### 17-11. 미실시 테스트

다음 항목은 현재 제출 자료에서 실제 상황을 발생시킨 별도의 실행 증빙이 확인되지 않아 `미실시`로 구분하였습니다.

- HTTP 429 API 쿼터 오류
- 네트워크 오류

위 항목은 구현된 오류 처리 로직과 예상 결과를 문서에 설명하되, 실제 테스트를 수행하지 않은 내용을 임의로 성공한 것으로 작성하지 않았습니다.
...
프로그램은 입력한 여행 날짜를 검증한 후 OpenAI API를 이용하여 여행지 추천을 수행하고, 추천된 각 지역의 `recommended_city` 값을 이용하여 장소 검색 단계로 연결합니다.

추천 지역명은 `normalize_city_name()` 함수를 이용하여 시·군 단위 지역명으로 정규화한 뒤 `PlaceProvider` 인터페이스를 구현한 `KakaoPlaceProvider`를 통해 Kakao Local API 맛집 검색에 사용합니다.
...
검색된 맛집 정보는 다음 항목으로 정리하여 `restaurants_by_city`에 지역별로 저장합니다.
...
`x`는 경도(Longitude), `y`는 위도(Latitude)를 의미하며 Kakao Local API에서 제공하는 값을 숫자형으로 변환하여 원본 JSON에 저장합니다.
...
### 18-2. 장소 검색 실패 및 검색 결과 0건 처리

Kakao Local API를 이용한 장소/맛집 검색 과정에서는 인증 또는 권한 오류, API 사용량 제한, 네트워크 오류 등의 문제가 발생할 수 있습니다.

프로그램에서는 장소 검색 API 자체가 실패한 경우와 API 요청은 정상적으로 수행되었지만 검색 결과가 0건인 경우를 구분하여 처리합니다.
...
원본 JSON에서는 맛집 검색에 실패한 지역의 데이터를 다음과 같은 구조로 빈 배열에 저장합니다.
...
맛집 검색 결과가 0건인 경우는 Kakao Local API 요청 자체는 정상적으로 처리되었지만 검색 조건에 해당하는 음식점이 조회되지 않은 상황입니다.

실제 `2026-09-18` 테스트에서는 맛집 검색 결과가 없는 상황을 발생시켜 다음과 같이 `데이터 없음`으로 처리되는 것을 확인하였습니다.
...
검색 결과가 0건인 경우 해당 지역의 맛집 목록은 빈 배열로 처리하고 프로그램을 종료하지 않은 채 다음 추천 지역의 처리를 계속합니다.
...
최종 여행 안내서 생성
        ↓
원본 JSON 및 Markdown 결과 저장
```

실제 테스트에서는 맛집 검색 결과가 0건인 상황에서도 프로그램이 다음 추천 지역의 처리를 계속하였으며, 최종적으로 다음 결과 파일이 정상 저장되는 것을 확인하였습니다.
...
**그림 18-2-1. 맛집 검색 결과 0건 처리 및 다음 추천 지역 처리**

![맛집 검색 결과 0건 최종 완료](evidence/27.%20맛집검색0건처리Test결과%20(02).png)

**그림 18-2-2. 맛집 검색 결과 0건 발생 후 최종 결과 저장 완료**

따라서 장소 검색과 관련하여 현재 실제 테스트로 확인된 항목은 다음과 같습니다.

- HTTP 403 Forbidden 오류 처리
- 맛집 검색 결과 0건 처리
- 검색 결과 0건 발생 후 다음 추천 지역 처리 계속
- 검색 결과 0건 발생 후 최종 JSON 및 Markdown 결과 저장

반면 HTTP 429 API 쿼터 오류와 실제 네트워크 연결 실패 또는 Timeout 상황은 별도의 실행 테스트를 수행하지 않았으므로 `미실시`로 구분합니다.
...
OpenAI API에서는 사용자가 입력한 여행 날짜를 바탕으로 서로 다른 국내 여행지 3곳을 추천받고, 각 추천 결과를 `recommended_city`, `weather`, `events`, `reason` 등의 구조화된 데이터로 처리하였습니다.

추천된 지역의 `recommended_city` 값은 다음 장소 검색 단계의 입력값으로 연결하였습니다.

이 과정에서 LLM이 반환하는 지역명이 항상 동일한 형식이라는 보장이 없기 때문에 `normalize_city_name()` 함수를 이용하여 긴 행정구역명을 시·군 단위의 지역명으로 정규화한 뒤 장소 검색에 사용하도록 구성하였습니다.

예를 들어 다음과 같이 지역명을 변환할 수 있도록 하였습니다.
...
프로그램에서는 API Key 미설정, LLM 구조화 결과 처리 실패, 장소 API 인증 및 권한 오류, API 사용량 제한, 네트워크 오류, 서버 오류 및 검색 결과 0건 등의 상황을 구분하여 처리하도록 구성하였습니다.
...
LLM의 여행지 추천 결과를 정상적인 구조화 형태로 처리하지 못한 경우에는 무한 반복하지 않고 최대 1회만 재시도하도록 구성하였습니다. 재시도 시에는 동일한 요청을 그대로 반복하지 않고 보정 Prompt를 추가하여 필수 Key, 자료형 및 추천 항목 수를 다시 명확하게 지정하도록 구현하였습니다.

보정 Prompt를 적용한 재시도에서도 구조화된 결과를 처리하지 못하면 프로그램을 종료하도록 구성하였습니다.
...
반면 HTTP 429 API 쿼터 오류와 네트워크 오류는 처리 로직을 구현하였지만 별도의 실제 테스트 증빙은 확보하지 않았으므로 테스트 결과에서는 `미실시`로 구분하였습니다.

맛집 검색 결과 0건은 실제 테스트를 수행하였습니다. `2026-09-18` 실행에서 특정 추천 지역의 맛집 검색 결과가 없는 경우 해당 지역의 맛집 정보를 `데이터 없음`으로 처리하고, 프로그램이 중단되지 않은 채 다음 추천 지역의 처리를 계속하는 것을 확인하였습니다.

또한 맛집 검색 결과가 0건인 상황에서도 최종 여행 안내서 생성 단계까지 정상적으로 진행되었으며, 다음 원본 JSON 및 Markdown 결과 파일이 정상적으로 저장되는 것을 확인하였습니다.
...
LLM의 여행지 추천 결과를 정상적인 구조화 형태로 처리하지 못하는 경우에는 무한 재시도를 방지하기 위해 최대 1회만 다시 요청하도록 구성하였습니다.

최신 구현에서는 동일한 요청을 그대로 반복하지 않고, 첫 번째 구조화 결과 처리에 실패하면 재시도용 보정 Prompt를 추가합니다.

재시도 시에는 다음 조건을 다시 명확하게 지정하도록 구성하였습니다.
...
OpenAI API에서 반환되는 `recommended_city` 값은 경우에 따라
`강원특별자치도 강릉시`, `경상북도 경주시`처럼 광역 행정구역명이
포함된 형태일 수 있습니다.

이 값을 그대로 Kakao Local API 검색어로 사용할 경우 검색어가
불필요하게 길어질 수 있으므로, `normalize_city_name()` 함수를
이용하여 시 또는 군 단위 지역명으로 정규화하도록 구현하였습니다.
...
도시명 정규화 함수를 직접 실행하여 긴 행정구역명이 시·군 단위
지역명으로 정상 변환되는지 확인하였습니다.

![도시명 정규화 테스트 결과](evidence/21-1_도시명정규화_테스트결과.png)

**그림 21-1. 도시명 정규화 함수 실행 및 변환 결과**

실행 결과 다음과 같이 정상적으로 변환되는 것을 확인하였습니다.
...
이를 통해 LLM이 광역 행정구역명을 포함한 긴 지역명을 반환하더라도
`normalize_city_name()`을 통해 시·군 단위 지역명으로 정규화한 후
Kakao Local API의 맛집 검색에 사용할 수 있도록 구현하였습니다.
...
### 21-3. LLM 파싱 실패 시 보정 Prompt 적용 및 1회 재시도

LLM의 여행지 추천 결과를 구조화된 형태로 처리하지 못한 경우 동일한 요청을 그대로 반복하지 않고, 재시도용 보정 Prompt를 추가하여 최대 1회 다시 요청하도록 구현하였습니다.

첫 번째 요청에서는 기본 여행지 추천 Prompt를 사용합니다.

구조화된 결과를 읽지 못하거나 JSON 파싱에 실패하면 다음과 같은 메시지를 출력합니다.

```text
LLM JSON 파싱 실패: 보정 프롬프트를 적용하여 1회 재시도합니다.
```

두 번째 요청에서는 기존 Prompt에 다음 조건을 추가하여 출력 형식을 다시 명확하게 지정합니다.

- 설명 문장이나 Markdown 코드블록을 추가하지 않음
- `recommendations`에 정확히 3개의 항목을 포함
- 각 항목에 `recommended_city`, `weather`, `events`, `reason` 필수
- `recommended_city`, `weather`, `reason`은 문자열
- `events`는 문자열 배열
...
재시도 후에도 구조화된 결과를 읽지 못하면 프로그램을 종료하도록 구성하였습니다.

이를 통해 단순히 동일한 요청을 반복하는 것이 아니라, 첫 번째 응답을 처리하지 못한 경우 두 번째 요청에서 필요한 필드와 자료형을 다시 명확하게 지정하여 구조화된 응답을 받을 가능성을 높이도록 구성하였습니다.

반면 HTTP 429 또는 503 등의 API 서버 상태 코드에 대해 별도의 자동 재시도 기능을 구현하지 않았으므로, 해당 기능을 구현했다고 작성하지 않습니다.


### 21-4. 장소 검색 API Provider 추상화

장소 검색 기능이 특정 지도 API 구현에 직접 의존하지 않도록 `PlaceProvider` 공통 인터페이스를 정의하였습니다.

현재 Kakao Local API는 `KakaoPlaceProvider` 클래스가 `PlaceProvider`를 구현하는 방식으로 구성되어 있습니다.
...
`PlaceProvider`에서는 장소 검색 Provider가 공통으로 제공해야 하는 `search_restaurants()` 메서드를 정의합니다.
...
현재 Kakao Local API에 대한 실제 장소 검색은 `KakaoPlaceProvider`에서 처리합니다.
...
`main()`에서는 Kakao 전용 검색 함수를 직접 호출하지 않고 `PlaceProvider` 형식의 객체를 생성하여 사용합니다.
...
맛집 검색 시에는 도시명을 정규화한 후 Provider를 통해 검색합니다.
...
Kakao Local API의 검색 결과에서는 다음 정보를 `Restaurant` 구조로 저장합니다.
...
`x`는 경도(Longitude), `y`는 위도(Latitude)를 의미하며 원본 JSON에 장소 위치 좌표를 저장하기 위해 사용합니다.

따라서 향후 다른 장소 검색 API를 사용할 경우 동일한 `PlaceProvider` 인터페이스를 구현하는 새로운 Provider를 추가하고 실행 시 Provider 객체를 교체하면, 여행지 추천, 복수 지역 반복 처리, 결과 저장 및 최종 여행 안내서 생성 로직을 크게 수정하지 않고 장소 검색 API를 교체할 수 있도록 구성하였습니다.


### 21-5. LLM 및 장소 검색 기능 모듈화

OpenAI API와 장소 검색 API를 사용하는 기능은 프로그램의 처리 목적에 따라 역할을 분리하여 구성하였습니다.

먼저 여행 날짜를 바탕으로 여행지를 추천하고 구조화된 결과를 생성하는 단계와, 추천 지역명을 정규화하여 장소 검색 API에 전달하는 단계, 여행지 추천 정보 및 지역별 맛집 검색 결과를 이용하여 최종 여행 안내서를 생성하는 단계를 구분하였습니다.

특히 다음 기능을 각각 독립된 함수 또는 클래스로 분리하였습니다.

- `get_travel_recommendation()` : 여행 날짜를 이용한 LLM 여행지 추천
- `normalize_city_name()` : 긴 행정구역명을 시·군 단위 지역명으로 정규화
- `PlaceProvider` : 장소 검색 API 공통 인터페이스
- `KakaoPlaceProvider` : Kakao Local API 맛집 검색 구현
- `generate_travel_guide()` : 여행지 추천 정보와 맛집 정보를 이용한 최종 Markdown 여행 안내서 생성
...
복수 지역 추천에서는 `recommendations`에 저장된 추천 지역을 하나씩 순회하면서 같은 장소 검색 Provider를 반복하여 사용합니다.
...
이를 통해 여행지 추천, 도시명 정규화, 장소 검색, 최종 여행 안내서 생성의 역할을 구분하여 프로그램의 처리 흐름을 확인하기 쉽고, 특정 기능을 수정하거나 교체할 때 다른 부분에 미치는 영향을 줄일 수 있도록 구성하였습니다.


### 21-6. 원본 JSON 결과 구조 및 캐싱

프로그램의 원본 JSON에는 실행 결과를 확인할 수 있도록 여행 날짜, 여행지 추천 결과, 지역별 맛집 검색 결과 및 오류 정보를 구분하여 저장하였습니다.
...
`travel_date`를 통해 어떤 날짜를 기준으로 생성된 결과인지 확인할 수 있습니다.

`recommendations`에는 OpenAI API에서 생성한 여행지 추천 결과가 저장되며, 각 추천 결과에는 다음 정보가 포함됩니다.
...
`restaurants_by_city`에는 추천 지역명을 기준으로 지역별 맛집 검색 결과를 구분하여 저장합니다.

각 맛집 정보에는 다음 항목이 포함됩니다.
...
`x`, `y`는 Kakao Local API에서 제공하는 위치 좌표이며 프로그램에서 숫자형으로 변환하여 원본 JSON에 저장합니다.

또한 실행 과정에서 발생한 비치명적 오류는 `errors` 배열에 기록합니다.
...
오류가 없는 경우에도 `errors`는 빈 배열 형태로 유지합니다.

동일한 여행 날짜의 원본 JSON과 Markdown 여행 안내서가 이미 존재하는 경우에는 `load_cached_results()`를 이용하여 기존 결과를 불러오도록 구현하였습니다.

캐시 사용 시에는 다음 API 호출을 다시 수행하지 않습니다.

- OpenAI API 여행지 추천
- Kakao Local API 맛집 검색
- OpenAI API 최종 여행 안내서 생성

캐시에서 기존 결과를 읽을 때는 다음 정보와 파일 경로를 함께 반환하도록 구성하였습니다.
...
이를 통해 `main()`에서 동일한 결과 파일 경로를 다시 계산하지 않고 기존 캐시 결과와 저장 경로를 그대로 사용할 수 있도록 구성하였습니다.

다만 생성 시각, `cache_used`, 전체 맛집 개수 등의 정보를 별도의 `metadata` 객체로 저장하는 기능은 구현하지 않았으므로, 해당 기능을 구현했다고 작성하지 않습니다.
...
| 05 | `evidence/11_LLM_JSON파싱실패_재시도.png` | LLM 구조화 결과 처리 실패 후 보정 Prompt를 적용하여 최대 1회 재시도하는 결과 확인 |
...
| 17 | `evidence/21-1_도시명정규화_테스트결과.png` | 긴 행정구역명을 시·군 단위 지역명으로 정규화하는 기능 확인 |
| 18 | `evidence/27. 맛집검색0건처리Test결과 (01).png` | Kakao Local API 맛집 검색 결과가 0건인 경우 `데이터 없음`으로 처리하고 다음 지역 처리를 계속하는 결과 확인 |
| 19 | `evidence/27. 맛집검색0건처리Test결과 (02).png` | 맛집 검색 결과 0건 발생 후에도 최종 JSON과 Markdown 여행 안내서가 정상 저장되는 결과 확인 |
| 20 | `evidence/28-1_날짜형식오류.png` | `YYYY-MM-DD` 형식이 아닌 날짜 입력 시 올바른 형식을 안내하고 API 호출 전에 프로그램이 종료되는 결과 확인 |
| 21 | `evidence/28-2_-date옵션누락오류.png` | 필수 `-date` 옵션 누락 시 CLI 사용 방법과 필수 인자 오류가 출력되고 API 호출 전에 프로그램이 종료되는 결과 확인 |
...
- `normalize_city_name()`을 이용한 도시명 정규화
- 정규화된 도시명을 이용한 Kakao Local API 맛집 검색
- `PlaceProvider`와 `KakaoPlaceProvider`를 이용한 장소 검색 API 구조 분리
- 지역별 맛집 정보를 `restaurants_by_city`에 저장
- 맛집 정보의 `place_name`, `address`, `category`, `url`, `x`, `y` 저장
- 최종 Markdown 여행 안내서 생성
- 오전·오후·저녁 일정 생성
- API Key 미설정 오류 처리
- LLM 구조화 결과 처리 실패 후 보정 Prompt를 적용한 최대 1회 재시도
- Kakao Local API HTTP 403 오류 처리
- 맛집 검색 결과 0건 발생 시 `데이터 없음` 처리
- 맛집 검색 결과 0건 발생 후 다음 추천 지역 처리 계속
- 맛집 검색 결과 0건 발생 후 최종 JSON 및 Markdown 결과 정상 저장
- 잘못된 날짜 형식 입력 시 API 호출 전 오류 처리
- 필수 `-date` 옵션 누락 시 CLI 사용 방법 및 필수 인자 오류 출력
- 동일 날짜 결과 캐싱
...
21-1_도시명정규화_테스트결과.png
27. 맛집검색0건처리Test결과 (01).png
27. 맛집검색0건처리Test결과 (02).png
28-1_날짜형식오류.png
28-2_-date옵션누락오류.png
```
...
- [x] 날짜 형식 오류를 실제 실행하여 오류 메시지를 확인함
- [x] 필수 `-date` 옵션 누락을 실제 실행하여 사용법 또는 오류 메시지를 확인함
...
- [x] `recommended_city` 값을 다음 장소 검색 단계의 입력값으로 사용함
- [x] LLM JSON 파싱 또는 구조화 결과 처리 실패 시 최대 1회만 재시도하도록 구현함
- [x] 재시도 시 보정 Prompt를 추가하여 필수 Key, 자료형 및 추천 항목 수를 다시 명확하게 지정함
- [x] 재시도 결과를 `TravelRecommendations` 구조로 검증하도록 구현함
- [x] LLM JSON 파싱 또는 구조화 결과 처리 실패 후 1회 재시도 테스트를 수행함

### 23-3. Kakao Local API 및 맛집 검색

- [x] Kakao Local API가 프로그램에 연결되어 있음
- [x] `PlaceProvider` 공통 인터페이스를 이용하여 장소 검색 API를 추상화함
- [x] `KakaoPlaceProvider`에서 Kakao Local API 맛집 검색을 구현함
- [x] `normalize_city_name()`을 이용하여 긴 행정구역명을 시·군 단위 지역명으로 정규화함
- [x] 정규화된 도시명을 `PlaceProvider`의 맛집 검색 입력값으로 사용함
- [x] 도시명 정규화 기능을 실제 실행하여 변환 결과를 확인함
- [x] 추천 지역명을 이용하여 `추천지역 + 맛집` 형식의 검색어를 생성함
- [x] 음식점 카테고리 코드 `FD6`를 사용함
- [x] 지역별 최대 5곳의 맛집을 검색하도록 구현함
- [x] 맛집 정보에서 `place_name`, `address`, `category`, `url`, `x`, `y`를 저장함
- [x] `x`, `y` 위치 좌표를 숫자형으로 변환하여 원본 JSON에 저장함
...
- [x] 맛집 검색 결과 0건 상황을 실제 실행하여 확인함
- [x] 맛집 검색 결과 0건 발생 후에도 다음 지역 처리를 계속하는 것을 확인함
- [x] 맛집 검색 결과 0건 발생 후에도 최종 결과 파일이 정상 저장되는 것을 확인함
...
- [x] LLM JSON 파싱 또는 구조화 결과 처리 실패 시 최대 1회만 재시도함
- [x] LLM 재시도 시 보정 Prompt를 적용함
...
- [x] 맛집 검색 결과가 0건인 경우 프로그램을 중단하지 않고 빈 목록으로 처리함
...
```text
results/2026-09-19_raw.json
results/2026-09-19_travel_guide.md
```
...
- [x] 제주시, 강릉시, 경주시의 복수 지역 추천 실행 결과를 확인함
...
현재 구현 및 증빙자료를 기준으로 주요 기능과 실제 테스트 항목은 대부분 확인되었습니다.
